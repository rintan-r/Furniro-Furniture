  ...
  return (
    <section className='grid grid-cols-1 md:grid-cols-[553px_606px] justify-between pl-[99px] pr-[99px] pt-[35px] pb-[55px]'>
      ...
      <div className='flex flex-col gap-4 order-3'>
        <div className='h-8 w-2/3 bg-amber-200 rounded'> {detail.title} </div> {/* Title */}
        <div className='h-6 w-1/3 bg-amber-200 rounded'> {detail.price} </div> {/* Price */}
        <div className='h-6 w-1/3 bg-amber-200 rounded'> {detail.originalPrice} </div> {/* Price */}
        <div className='h-16 w-full bg-amber-200 rounded'> {detail.detailDescription} </div>{/* Description */}
        <div className='h-10 w-1/2 bg-amber-200 rounded'> {detail.color} </div>{/* Size */}
        <div className='h-10 w-1/2 bg-amber-200 rounded'> {detail.SKU} </div>{/* Color */}
        <div className='h-12 w-full bg-amber-200 rounded'> {detail.category} </div>{/* Add to Cart */}
        <div className='h-12 w-full bg-amber-200 rounded'> {detail.tags} </div>{/* Add to Cart */}
